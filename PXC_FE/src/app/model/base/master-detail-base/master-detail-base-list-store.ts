...
@Injectable()
export class MasterDetailBaseListStore extends ComponentStore<MasterDetailListState> {
  constructor() {
    super({
   ...
   * Created by: MTLUC - 02/11/2021
   */
   readonly summaryData$: Observable<any> = this.select(
   ...
   * Created by: MTLUC - 02/11/2021
   */
  readonly setSummaryData = this.updater((state, summaryData: any) => {
   ...
   * Created by: MTLUC - 02/11/2021
   */
  readonly search$: Observable<any> = this.select((state) => state.search);

  /*
   * set search
   * Created by: MTLUC - 02/11/2021
   */
  readonly setSearch = this.updater((state, search: any) => {
    return { ...state, search };
  });
  //#endregion
  ...
  readonly loadDataComplete = this.updater(
    (
      ...
      return {
        ...
        summaryData:newState.summaryData
      };
    }
  );
