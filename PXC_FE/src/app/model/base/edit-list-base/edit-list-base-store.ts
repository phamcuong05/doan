...
export interface EditListBaseState {
  currentRow?: object;
  sortable: boolean;
  filterable: boolean;
...
@Injectable()
export class EditListBaseStore extends ComponentStore<EditListBaseState> {
  constructor() {
    super({
   ...
   * Created by: MTLUC - 02/11/2021
   */
   readonly idField$: Observable<any> = this.select(
    (state) => state.idField
  );
   ...
   * Created by: MTLUC - 02/11/2021
   */
  readonly setIdField = this.updater((state, idField: string) => {
